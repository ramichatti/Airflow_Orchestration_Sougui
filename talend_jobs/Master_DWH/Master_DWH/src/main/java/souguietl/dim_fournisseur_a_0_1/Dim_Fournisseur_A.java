...
package souguietl.dim_fournisseur_a_0_1;
...
/**
 * Job: Dim_Fournisseur_A Purpose: <br>
 * Description:  <br>
 * @author 
 * @version 8.0.1.20211109_1610
 * @status 
 */
public class Dim_Fournisseur_A implements TalendJob {
...
protected static void logIgnoredError(String message, Throwable cause) {
	...
	private final String jobName = "Dim_Fournisseur_A";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Dim_Fournisseur_A.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Dim_Fournisseur_A.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class ajoutStruct implements routines.system.IPersistableRow<ajoutStruct> {
    final static byte[] commonByteArrayLock_SOUGUIETL_Dim_Fournisseur_A = new byte[0];
    static byte[] commonByteArray_SOUGUIETL_Dim_Fournisseur_A = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_SOUGUIETL_Dim_Fournisseur_A.length) {
				if(length < 1024 && commonByteArray_SOUGUIETL_Dim_Fournisseur_A.length == 0) {
   					commonByteArray_SOUGUIETL_Dim_Fournisseur_A = new byte[1024];
				} else {
   					commonByteArray_SOUGUIETL_Dim_Fournisseur_A = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_SOUGUIETL_Dim_Fournisseur_A, 0, length);
			strReturn = new String(commonByteArray_SOUGUIETL_Dim_Fournisseur_A, 0, length, utf8Charset);
		}
		return strReturn;
	}
	...
	private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException{
		...
		if (length == -1) {
			...
			unmarshaller.readFully(commonByteArray_SOUGUIETL_Dim_Fournisseur_A, 0, length);
			strReturn = new String(commonByteArray_SOUGUIETL_Dim_Fournisseur_A, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_SOUGUIETL_Dim_Fournisseur_A) {
...
String dbUser_tDBOutput_1 = null;
	...
	final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:spOngiqdn7V4f5ngpkb068rshKcCB8XKCrEC/Ul1synzq1gi");
...
if(dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
	...
	int count_tDBOutput_1=0;
        ...
        String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1 + "] ([nom_fournisseur],[matricule_fiscal],[telephone]) VALUES (?,?,?)";
...
String dbUser_tDBOutput_2 = null;
	...
	final String decryptedPassword_tDBOutput_2 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:sxVt9od+qUOA9Xqd716hv+I49R+ydHqAcM5OC7ZNeCPo5X+o");
...
if(dbschema_tDBOutput_2 == null || dbschema_tDBOutput_2.trim().length() == 0) {
    tableName_tDBOutput_2 = "Dim_fournisseur";
} else {
    tableName_tDBOutput_2 = dbschema_tDBOutput_2 + "].[" + "Dim_fournisseur";
}
	int count_tDBOutput_2=0;

        String update_tDBOutput_2 = "UPDATE [" + tableName_tDBOutput_2 + "] SET [matricule_fiscal] = ?,[nom_fournisseur] = ?,[telephone] = ? WHERE [id_fournisseur] = ?";
 ...
 */
	...
	final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:6/NQk93U1BYG9L65U1/Fl/rlyeI0UvcPZYlsT129X9gQA0jh");
		    ...
		    String dbquery_tDBInput_1 = "SELECT \n    MAX(fournisseur) AS fournisseur,\n    matricule_fiscal,\n    LEFT(MAX(telephones), \n         CHARINDEX(',"
+"', MAX(telephones) + ',') - 1) AS telephone\nFROM Sougui_SA.dbo.Entete_Achat_SA\nGROUP BY matricule_fiscal;";
...
// # Output table : 'out1'
out1_tmp.fournisseur = row1.fournisseur.replace("Ã‰", "É");
out1_tmp.matricule_fiscal = row1.matricule_fiscal ;
out1_tmp.telephone = (row1.telephone == null || "Non trouvé".equals(row1.telephone)) ? "" : row1.telephone.replace("+216", "").trim() ;
out1 = out1_tmp;
// ###############################
 					    ...
 					    boolean forceLooprow2 = false;
 							...
 							row2Struct row2ObjectFromLookup = null;
								...
								hasCasePrimitiveKeyWithNull_tMap_2 = false;
                        		    		    ...
                        		    		    row2HashKey.matricule_fiscal = out1.matricule_fiscal ;
...
ajout_tmp.nom_fournisseur = out1.fournisseur ;
ajout_tmp.matricule_fiscal = out1.matricule_fiscal ;
ajout_tmp.telephone = out1.telephone ;
ajout = ajout_tmp;
...
modify_tmp.id_fournisseur = row2.id_fournisseur ;
modify_tmp.matricule_fiscal = out1.matricule_fiscal ;
modify_tmp.nom_fournisseur = out1.fournisseur ;
modify_tmp.telephone = out1.telephone ;
modify = modify_tmp;
} // closing filter/reject
// ###############################
...
if(ajout != null) { 
	...
	currentComponent="tDBOutput_1";
        ...
        whetherReject_tDBOutput_1 = false;
                    if(ajout.nom_fournisseur == null) {
pstmt_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(1, ajout.nom_fournisseur);
}
...
pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
                    ...
                    if(ajout.telephone == null) {
pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_1.setString(3, ajout.telephone);
}
...
} else {pstmt_tDBOutput_2.setString(1, modify.matricule_fiscal);
}

                    if(modify.nom_fournisseur == null) {
pstmt_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(2, modify.nom_fournisseur);
}

                    if(modify.telephone == null) {
pstmt_tDBOutput_2.setNull(3, java.sql.Types.VARCHAR);
} else {pstmt_tDBOutput_2.setString(3, modify.telephone);
}


                    pstmt_tDBOutput_2.setInt(4 + count_tDBOutput_2, modify.id_fournisseur);
 ...
 final boolean execStat = this.execStat;
	...
	currentComponent="tAdvancedHash_row2";
		...
		int tos_count_tAdvancedHash_row2 = 0;
			   		...
			   		// linked node: tMap_2 - inputs:(out1,row2) outputs:(ajout,modify)
 ...
 */
	...
	final String decryptedPassword_tDBInput_2 = routines.system.PasswordEncryptUtil.decryptPassword("enc:routine.encryption.key.v1:KPr9QJEDI8s8+zfiCMcYH4EZb2s4tnu/KIGa4WPjSliWjFv/");
...
+"r,\n		dbo.Dim_fournisseur.telephone\nFROM	dbo.Dim_fournisseur";
		    ...
		    while (rs_tDBInput_2.next()) {
		        nb_line_tDBInput_2++;
							...
							if(colQtyInRs_tDBInput_2 < 1) {
								row2.id_fournisseur = 0;
							} else {
		                          
            row2.id_fournisseur = rs_tDBInput_2.getInt(1);
            ...
            if(tmpContent_tDBInput_2 != null) {
            	if (talendToDBList_tDBInput_2 .contains(rsmd_tDBInput_2.getColumnTypeName(3).toUpperCase(java.util.Locale.ENGLISH))) {
            		row2.nom_fournisseur = FormatterUtils.formatUnwithE(tmpContent_tDBInput_2);
            	} else {
                	row2.nom_fournisseur = tmpContent_tDBInput_2;
                }
            } else {
                row2.nom_fournisseur = null;
            }
            ...
            if(tmpContent_tDBInput_2 != null) {
            	if (talendToDBList_tDBInput_2 .contains(rsmd_tDBInput_2.getColumnTypeName(4).toUpperCase(java.util.Locale.ENGLISH))) {
            		row2.telephone = FormatterUtils.formatUnwithE(tmpContent_tDBInput_2);
            	} else {
                	row2.telephone = tmpContent_tDBInput_2;
                }
            } else {
                row2.telephone = null;
            }
 ...
 */
	...
	currentComponent="tAdvancedHash_row2";
				...
				row2_HashRow.id_fournisseur = row2.id_fournisseur;
				
				row2_HashRow.matricule_fiscal = row2.matricule_fiscal;
				
				row2_HashRow.nom_fournisseur = row2.nom_fournisseur;
 ...
 */
    ...
    public static void main(String[] args){
        final Dim_Fournisseur_A Dim_Fournisseur_AClass = new Dim_Fournisseur_A();

        int exitCode = Dim_Fournisseur_AClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Dim_Fournisseur_A.class.getClassLoader().getResourceAsStream("souguietl/dim_fournisseur_a_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Dim_Fournisseur_A.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
                try {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Dim_Fournisseur_A");
        }
...
/************************************************************************************************
 *     120508 characters generated by Talend Open Studio for Data Integration 
